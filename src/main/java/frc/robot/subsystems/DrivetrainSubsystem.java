...
public class DrivetrainSubsystem extends TunerSwerveDrivetrain implements Subsystem {
    ...
    /**
     * Create a command that will drive to the specified Pose2d. Does not
     * end on its own; you must add your own ending condition.
     * @param target The target Pose2d to drive to.
     */
    public DriveToPose driveTo(Pose2d target) {
        return new DriveToPose(this, target);
    }

    /**
     * Create a command that will drive to the Pose2d provided by the
     * targetSupplier. This allows you to calculate the target dynamically
     * at the time when the command is initialized (started).
     * Does not end on its own; you must add your own ending condition.
     */
    public DriveToPose driveTo(Supplier<Pose2d> targetSupplier) {
        return new DriveToPose(this, targetSupplier);
    }

    /**
     * Creates a command that will drive forward a distance in meters,
     * and ends when it reaches that distance.
     */
    public Command driveDistance(double meters) {
        return driveToRelativeTransform(new Transform2d(meters, 0, Rotation2d.kZero));
    }

    /**
     * Creates a command that will turn in place a number of degrees,
     * and ends when it reaches that angle.
     */
    public Command turnDegrees(double degrees) {
        return driveToRelativeTransform(new Transform2d(0, 0, Rotation2d.fromDegrees(degrees)));
    }

    private Command driveToRelativeTransform(Transform2d transform) {
        final Pose2d[] cmdTarget = new Pose2d[1];
        return driveTo(() -> {
            return cmdTarget[0] = getPose().transformBy(transform);
        })
        .finallyDo(() -> drive(new ChassisSpeeds()))
        .until(() -> Helpers.withinTolerance(getPose(), cmdTarget[0], 0.05, 0.05, 2.0, false, false, false));
    }
